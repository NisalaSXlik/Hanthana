document.addEventListener('DOMContentLoaded', () => {
    ...
    let discoverEmptyState = null;

    if (discoverSearchInput && discoverGrid) {
        discoverSearchInput.addEventListener('input', () => {
            const query = (discoverSearchInput.value || '').trim().toLowerCase();
            let visibleCount = 0;

            tiles.forEach((tile) => {
                const searchable = (tile.getAttribute('data-search') || '').toLowerCase();
            ...
            if (visibleCount === 0) {
                if (!discoverEmptyState) {
                    discoverEmptyState = document.createElement('p');
                    discoverEmptyState.className = 'discover-search-empty';
                    discoverGrid.appendChild(discoverEmptyState);
                }
                discoverEmptyState.textContent = 'No matching posts found.';
            } else if (discoverEmptyState) {
                discoverEmptyState.remove();
                discoverEmptyState = null;
            }
        });
    }
    ...
    function createPostElement(post) {
        ...
        el.addEventListener('click', (e) => {
            const contentClickTarget = e.target.closest('.body .photo, .body .post-image');
            if (!contentClickTarget) return;
