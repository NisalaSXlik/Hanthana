...
function initializeEventHandlers() {
    ...
    // Create event button – reuse the shared post modal (from navbar)
    // and auto-select "Event Post" type so the user lands right on the event form.
    const createBtn = document.getElementById('createEventBtn');
    if (createBtn) {
        createBtn.addEventListener('click', function(e) {
            e.preventDefault();
            openPostModalAsEvent();
        });
    }
}
...
/**
 * Open the shared post-creation modal (from navbar) pre-set to "Event Post" mode.
 * This reuses the same form that the general Create button uses, so both
 * the feed and the events page go through PostsController for event creation.
 */
function openPostModalAsEvent() {
    const postModal = document.getElementById('postModal');
    if (!postModal) {
        showNotification('Post modal not found – please refresh the page.', 'error');
        return;
    }

    // Click the "Event Post" type button so the event-specific fields appear
    const eventTypeBtn = postModal.querySelector('.post-type-btn[data-type="event"]');
    if (eventTypeBtn) {
        eventTypeBtn.click();
    }
